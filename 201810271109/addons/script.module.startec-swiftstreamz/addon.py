...
def list_channels(cat_id=None):
    ...
    for ch in res['LIVETV']:
        ...
        try:
            ...
            pass
        url = plugin.url_for(play_id, cat_id=cat_id, channel_id=ch.get('id'))
...
@plugin.route('/play_id/<cat_id>/<channel_id>')
def play_id(cat_id, channel_id):
    r = s.get(list_url.format(cat_id), headers={'User-Agent': user_agent}, timeout=10)
    res = r.json(strict=False)
    ...
    if 'playlist.m3u8' in channel_url:
        media_url = '{0}?wmsAuthSign={1}|User-Agent={2}'.format(channel_url, auth_token, quote(data['DATA'][0]['Agent']))
        ...
        media_url = '{0}|User-Agent={1}'.format(channel_url, quote(data['DATA'][0]['Agent']))
        li = ListItem(title, path=media_url)
        li.setArt({'thumb': image, 'icon': image})
