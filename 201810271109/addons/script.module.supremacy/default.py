...
kidsurl    = base64.b64decode ('https://raw.githubusercontent.com/sClarkeIsBack/StreamHub/master/Links/Kids/kidscorner.xml')
docurl     = 'http://documentaryheaven.com'
mov2       = 'http://zmovies.to'
wwe        = 'http://watchwrestling.in'
tv         = base64.b64decode ('https://raw.githubusercontent.com/sClarkeIsBack/StreamHub/master/Links/247/247tv.xml')
	...
	if xbmcaddon.Addon().getSetting('enablexxxpass')=='true':
		...
		if (kb.isConfirmed()):
			...
			if pw == xbmcaddon.Addon().getSetting('xxxpass'):
				...
				xbmcgui.Dialog().ok('[COLOR red][B]supremacy[/B][/COLOR]','Incorrect Password, Please Try Again')
				return
	else:
		addDir("The Best Videos",xxxurl+'/best',24,icon,fanart,'')
		addDir("Latest Videos",xxxurl,24,icon,fanart,'')
...
def setxxxpass():
	d = xbmcgui.Dialog().yesno('[COLOR red]supremacy[/COLOR]','Would You Like To Set a Password for the XXX Section?')
	if d:
		...
		if (kb.isConfirmed()):
			pw = kb.getText()
			if pw =="":
				xbmcgui.Dialog().notification('[COLOR red]Password Cannot Be Blank[/COLOR]','supremacy')
				setxxxpass()
			else:
				xbmcaddon.Addon().setSetting('xxxpass',pw)
				xbmcaddon.Addon().setSetting('enablexxxpass','true')
				xbmcgui.Dialog().ok('[COLOR red]supremacy[/COLOR]','Password has been set')
...
def home():
	home = xbmc.executebuiltin('XBMC.RunAddon(plugin://plugin.video.supremacy/?action=)')
	return home
...
# OpenELEQ: query & type-parameter (added to line above)
	...
	if xbmcaddon.Addon('plugin.video.supremacy').getSetting('Username') == "":
		d = xbmcgui.Dialog().yesno('[COLOR ffff0000][B]supremacy Premium[/B][/COLOR]','Have You Donated And Would Like To Log-In?')
		if not d:
			showpremiumimage()
			sys.exit()
		else:
			from resources.premium import premium
			d = xbmcgui.Dialog().yesno('[COLOR ffff0000][B]supremacy Premium[/B][/COLOR]','Great! You will need to enter your Login details in the Addons Settings','Would you like us to open the settings for you now?')
			if d:
				xbmcaddon.Addon('plugin.video.supremacy').openSettings()
				premium.start('NEW')
			else:
				sys.exit()
	else:
